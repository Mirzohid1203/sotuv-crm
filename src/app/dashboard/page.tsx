...
export default function DashboardPage() {
  ...
  const activities = useMemo(() => {
    const dealActivities = deals.slice(0, 5).map(deal => ({
      id: `deal-${deal.id}`,
      text: `${deal.stage === "Closed Won" ? "Yopildi" : "Yangilandi"}: ${deal.title}`,
      subtext: deal.customerName ? `Mijoz: ${deal.customerName}` : `$${deal.value}`,
      time: deal.updatedAt?.toDate ? formatDistanceToNow(deal.updatedAt.toDate(), { addSuffix: true }) : "Hozirgina",
    ...
    const taskActivities = tasks.slice(0, 3).map(task => ({
      id: `task-${task.id}`,
      text: `Yangi vazifa: ${task.title}`,
      subtext: `Muddati: ${task.dueDate}`,
      time: "Hozirgina",
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        <Link href="/dashboard/pipeline" className="block">
          <StatCard 
            ...
            className="bg-indigo-600 text-white h-full"
          />
        </Link>
        ...
        <Card className="border-none shadow-xl bg-white/50 backdrop-blur-xl rounded-3xl overflow-hidden ring-1 ring-gray-200/50">
          <CardHeader className="border-b border-gray-100/50 bg-gray-50/30">
            <div className="flex items-center justify-between">
              ...
              <Link href="/dashboard/pipeline" className="text-indigo-600 text-xs font-bold hover:underline flex items-center gap-1">
            ...
            <div className="divide-y divide-gray-100">
              {activities.length > 0 ? (
                activities.map((activity) => (
                  <Link key={activity.id} href={activity.link} className="p-4 hover:bg-white transition-colors group cursor-pointer block">
